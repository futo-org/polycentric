...
class YoutubeTextVerifier extends TextVerifier {
  ...
  public async getClaimFieldsByUrl(url: string): Promise<Result<ClaimField[]>> {
    ...
    if (handleMatch) {
      ...
      if (!handle.startsWith('@')) {
        return Result.err({
          message: `Failed to find handle in URL '${url}'.`,
        });
      }
  ...
  private async getClaimFieldsByHandle(
    handle: string,
    ...
    if (response.status !== 200) {
      return Result.err({
        message: `Failed to get channel page from '${urlToFetch}'.`,
      });
    }

    const profileData = await response.text();
    const ogUrlMatch = /<meta property="og:url" content="([^"]+)">/.exec(
      profileData,
    );
    if (!ogUrlMatch) {
  ...
  private async getClaimFieldsByInternalId(
    internalId: string,
  ): Promise<Result<ClaimField[]>> {
