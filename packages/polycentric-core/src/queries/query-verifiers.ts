...
export type Callback = (verifiers: ReadonlySet<string>) => void;
...
export class QueryVerifiers {
  ...
  private queryStateToServers(
    ...
    for (const cell of queryState) {
      ...
      if (event.contentType.notEquals(Models.ContentType.ContentTypeVerifier)) {
        throw new Error('Incorrect content type for verifier event');
      }
  ...
  public query(
    ...
    const stateForSystem: StateForSystem = Util.lookupWithInitial(
      ...
      () => {
        ...
        const queryHandle = this.processHandle.queryManager.queryCRDTSet.query(
          this.processHandle.system(),
          Models.ContentType.ContentTypeVerifier,
          (patch) => {
            fulfilled.set();

            queryState.value = QueryIndex.applyPatch(queryState.value, patch);
...
export function queryVerifiersObservable(
