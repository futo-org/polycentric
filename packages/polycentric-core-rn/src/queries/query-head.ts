...
export class QueryHead implements HasUpdate {
    ...
    if (initial) {
      ...
      const subscription = RXJS.merge(...toMerge).subscribe((batch) => {
        if (batch.signedEvents.length > 0) {
          this.updateBatch(undefined, batch.signedEvents, batch.source);
      ...
      stateForSystem.unsubscribe = subscription.unsubscribe.bind(subscription);
    }
        ...
        RXJS.switchMap((processState) =>
          RXJS.from(
            ...
            RXJS.switchMap((potentialEvent) =>
              potentialEvent ? RXJS.of(potentialEvent) : RXJS.NEVER,
            ),
          ),
        ),
      );
    ...
    return RXJS.from(
      ...
      RXJS.switchMap((systemState) =>
        systemState.processes.length > 0
          ? RXJS.combineLatest(systemState.processes.map(loadProcessHead))
          : RXJS.of([]),
      ),
  ...
  private isStateMissingData(stateForSystem: StateForSystem): boolean {
    ...
    for (const [processString, headSignedEvent] of value.head.entries()) {
      ...
      const systemProcessesSignedEvent = value.processLists.get(processString);
  ...
  public updateBatch(
    ...
    for (const signedEvent of signedEvents) {
      ...
      if (
        event.contentType.equals(Models.ContentType.ContentTypeSystemProcesses)
      ) {
        const headSignedEvent = stateForSystem.value.head.get(processString);
        ...
        if (
          ...
          stateForSystem.value.processLists.set(processString, signedEvent);
          updatedStates.add(stateForSystem);
        }
      }
    }
