...
export function useIsAdmin(serverUrl: string): UseIsAdminResult {
    ...
    const { headers: authHeaders, loading: headersLoading, error: headersError, fetchHeaders } = useAuthHeaders(serverUrl);
            ...
            try {
                ...
                if (!checkResRaw.ok) {
                    if (checkResRaw.status === 401 || checkResRaw.status === 403) {
                        ...
                        let errorText = '';
                        try { errorText = await checkResRaw.text(); } catch (_) { /* Ignore */ }
                        throw new Error(`Admin check failed (@ ${serverUrl}): ${checkResRaw.status} ${checkResRaw.statusText}. Response: ${errorText}`);
                    }
                } else {
                    ...
                    try {
                        ...
                        console.error(`Error parsing JSON for admin check from ${serverUrl}:`, jsonError);
                        console.error(`Raw admin check response text from ${serverUrl}:`, checkResText);
                        throw new Error(`Invalid JSON admin check response received from ${serverUrl}`);
                    }
                }

            } catch (fetchError: any) {
                console.error(`Error checking admin status for ${serverUrl}:`, fetchError);
                setAdminCheckError(fetchError.message || `Failed to check admin status for ${serverUrl}.`);
                setIsAdmin(false);
            } finally {
                setCheckingAdmin(false);
            }
