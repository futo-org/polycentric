...
export const ForumBoardPage: React.FC = () => {
    const { serverUrl: encodedServerUrl, categoryId, boardId } = useParams<{ serverUrl: string; categoryId: string; boardId: string }>();
    ...
    const handleCreateThreadSubmit = async () => {
        ...
        try {
            ...
            if (!createRes.ok) {
                 ...
                 throw new Error(`Failed to create thread: ${createRes.status} ${createRes.statusText}`);
            }
                    ...
                    <div className="p-5 md:p-10 flex flex-col space-y-4">
                        ...
                        {isComposing && (
                            <div className="border p-4 rounded-md bg-gray-50 mb-4 space-y-3">
                                ...
                                <div>
                                     <label htmlFor="threadBody" className="block text-sm font-medium text-gray-700 mb-1">Body</label>
                                    <textarea
                                        id="threadBody"
                                        rows={5}
                                        value={newThreadBody}
                                ...
                                {createError && <p className="text-red-500 text-sm">Error: {createError}</p>}
                        ...
                        {!loading && !error && (
                            threads.length === 0 ? (
                                <p className="text-gray-500">No threads found in this board yet.</p>
                            ) : (
                                <ul className="space-y-3">
                                    {threads.map((thread) => (
                                        <li key={thread.id} className="border rounded-md hover:bg-gray-50 transition-colors">
                                            <Link 
                                                routerLink={`/forums/${encodedServerUrl}/${categoryId}/${boardId}/${thread.id}`}
                                                className="block group p-3" 
                                            >
                                                <h4 className="font-semibold text-blue-700 group-hover:underline">{thread.title}</h4>
