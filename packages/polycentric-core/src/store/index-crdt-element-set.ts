...
export class IndexCRDTElementSet implements HasIngest {
  ...
  public async ingest(
    ...
    const potentialExisting = potentialExistingAdd ?? potentialExistingRemove;
    ...
    if (
      ...
      parsedPotentialExisting.lessThan(event.lwwElementSet.unixMilliseconds)
    ) {
      const key =
        event.lwwElementSet.operation === Protocol.LWWElementSet_Operation.ADD
          ? addKey
          : removeKey;
      ...
      operations.push({
        type: 'put',
        key: key,
        value: new Uint8Array(event.lwwElementSet.unixMilliseconds.toBytesBE()),
