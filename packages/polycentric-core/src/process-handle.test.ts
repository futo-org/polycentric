...
describe('processHandle', () => {
  test('basic post', async () => {
    ...
    expect(Models.Event.fromBuffer(events[0].event).logicalClock).toStrictEqual(
      new Long(1, 0, true),
    );

    expect(Models.Event.fromBuffer(events[1].event).logicalClock).toStrictEqual(
      new Long(2, 0, true),
    );
  });
      ...
      expect(
        await processHandle
          ...
          .indexCRDTElementSet.queryIfAdded(
            ...
            Protocol.PublicKey.encode(subjectHandle.system()).finish(),
          ),
