...
describe('levelStoreEvent', () => {
    ...
    test('basic', async () => {
        ...
        expect(await state.levelRanges.values().all()).toStrictEqual([
            Protocol.StorageTypeRange.encode({
                publicKey: publicKey,
                writerId: writerId,
                lowSequenceNumber: 1,
                highSequenceNumber: 1,
            }).finish(),
        ]);

        expect(await state.levelEvents.values().all()).toStrictEqual([
            Protocol.Event.encode(event).finish(),
        ]);
    });

    test('followNew', async () => {
        const state = makeTestState();
        await DB.newIdentity(state);
        const event = makeFollowEvent(1);
        ...
        expect(await state.levelFollowing.values().all()).toStrictEqual([
    ...
    test('followFutureReplacesPast', async () => {
        const state = makeTestState();
        await DB.newIdentity(state);

        const firstEvent = makeFollowEvent(1);
        const secondEvent = makeFollowEvent(2);

        await DB.levelSaveEvent(state, firstEvent);
        await DB.levelSaveEvent(state, secondEvent);
        ...
        expect(await state.levelEvents.values().all()).toStrictEqual([
            Protocol.Event.encode(firstEvent).finish(),
            Protocol.Event.encode(secondEvent).finish(),
        ]);

        expect(await state.levelFollowing.values().all()).toStrictEqual([
            Protocol.StorageTypeFollowing.encode({
                publicKey: publicKey,
                unixMilliseconds: secondEvent.unixMilliseconds,
                unfollow: false,
            }).finish(),
        ]);
    });

    test('followPastDoesNotReplaceFuture', async () => {
        const state = makeTestState();
        await DB.newIdentity(state);
