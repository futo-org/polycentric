...
export const ReplyComposePopup = ({
  main,
  sub,
}: {
  main: {
  ...
  return (
    <div className="px-3 py-5 md:px-7 bg-white overflow-clip flex flex-col space-y-0 w-auto md:w-[40rem]">
      <div className="flex relative overflow-clip">
        <div className="mr-3 md:mr-4 flex-shrink-0 flex flex-col overflow-clip">
          ...
          <div className="flex-grow flex justify-center items-center py-5">
            <div className="border h-full min-h-[8rem]"></div>
          </div>
        </div>
      ...
      <div>
        <Compose hideTopic={true} maxTextboxHeightPx={250} />
      </div>
    </div>
  )
}

export const ReplyComposePopupFullscreen = ({
  main,
  sub,
}: {
  main: {
  ...
  return (
    <Modal setOpen={(b: boolean) => ({})}>
      <ReplyComposePopup main={main} sub={sub} />
    </Modal>
  )
}
