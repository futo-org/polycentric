...
export async function writeResult<T>(res: Response, result: Result<T>): Promise<void> {
    if (result.success) {
        ...
        res.status(result.error.statusCode ?? StatusCodes.INTERNAL_SERVER_ERROR).json({
...
export function handleBinaryOrJson(req: Request, res: Response, next: NextFunction): void {
...
export function httpResponseToError<T>(code: number, data: string, endpointName: string): Result<T> {
    switch (code) {
        case StatusCodes.INTERNAL_SERVER_ERROR: {
            return Result.err({
                message: 'Verifier encountered an error getting your profile information',
                extendedMessage: 'An error was encountered while getting the OAuth Token and Username',
            });
        }
...
export function getCallbackForPlatform(platform: Core.Models.ClaimType.ClaimType, uriEncode = false) {
    const url = `${process.env.OAUTH_CALLBACK_DOMAIN}/platforms/${platform.toInt().toString()}/oauth/callback`;
