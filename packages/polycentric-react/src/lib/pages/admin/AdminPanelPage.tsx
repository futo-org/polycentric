...
function DragHandle({ listeners, attributes }: { listeners: any, attributes: any }) {
    return (
         <button {...attributes} {...listeners} className="p-1 text-gray-400 hover:text-gray-600 cursor-grab touch-none">
...
export const AdminPanelPage: React.FC = () => {
    ...
    const serverUrl = encodedServerUrl ? decodeURIComponent(encodedServerUrl) : undefined;
    ...
    const [addingBoardToCategoryId, setAddingBoardToCategoryId] = useState<string | null>(null); // Track which category form is open
    ...
    const [editingCategory, setEditingCategory] = useState<Omit<Category, 'boards'> | null>(null); // Store category info without boards
    ...
    const overallLoading = adminLoading || loadingData || isAddingCategory || isAddingBoard || !!deletingBoardId || !!deletingCategoryId || isSavingBoard || isSavingCategory || headersLoading;
    // Combined error state - add saveCategoryError
    const overallError = adminError || dataError || addCategoryError || addBoardError || deleteBoardError || deleteCategoryError || saveBoardError || saveCategoryError || headersError;
    ...
    const fetchAdminData = useCallback(async () => {
        ...
        try {
            ...
            if (!authHeaders) {
                ...
                throw new Error("Could not get authentication headers to fetch admin data.");
            }
            ...
            const fetchedCategories: Omit<Category, 'boards'>[] = await catResponse.json();
            ...
            const categoriesWithBoards: Category[] = await Promise.all(
                fetchedCategories.map(async (category) => {
                    ...
                    try {
                        ...
                        const boardsWithCatId = boards.map(b => ({...b, category_id: category.id}));
                        return { ...category, boards: boardsWithCatId };
                    } catch (boardError) {
                        console.error(`[AdminPanelPage Fetch]   Error fetching boards for category ${category.id}:`, boardError);
                        return { ...category, boards: [] }; 
                    }
                })
            );
            ...
            categoriesWithBoards.forEach(cat => cat.boards.sort((a, b) => a.order - b.order));

            setCategories(categoriesWithBoards);
            setHasLoadedSuccessfully(true);

        } catch (error: any) { 
            console.error("[AdminPanelPage Fetch] ERROR caught in fetchAdminData:", error);
    ...
    const handleAddCategory = async (event: React.FormEvent<HTMLFormElement>) => {
        ...
        try {
            ...
            if (!response.ok) {
                ...
                try { errorText = await response.text(); } catch (_) {} // Ignore error reading text
    ...
    const handleDeleteBoard = async (boardId: string, boardName: string) => {
        // Simple confirmation
        if (!window.confirm(`Are you sure you want to delete the board "${boardName}"? This cannot be undone.`)) {
            return;
        }
    ...
    const handleDragEnd = (event: DragEndEvent) => {
        ...
        const isCategoryDrag = categories.some(cat => `category-${cat.id}` === activeId);
        ...
        setCategories((prevCategories) => {
            ...
            if (isCategoryDrag) {
                const oldIndex = newCategories.findIndex(cat => `category-${cat.id}` === activeId);
                const newIndex = newCategories.findIndex(cat => `category-${cat.id}` === overId);
                ...
                for (let i = 0; i < newCategories.length; i++) {
                   const boardIndex = newCategories[i].boards.findIndex(board => `board-${board.id}` === overId);
                   if (boardIndex !== -1) {
                ...
                // --- Find source category/board indices ---
                 for (let i = 0; i < newCategories.length; i++) {
                    const boardIndex = newCategories[i].boards.findIndex(board => `board-${board.id}` === activeId);
                    if (boardIndex !== -1) {
                 ...
                 const actualBoardToMove = prevCategories[sourceCategoryIndex]?.boards[sourceBoardIndex]; 
                ...
                if (sourceCategoryIndex === targetCategoryIndex) {
                    ...
                    const finalTargetBoardIndex = targetBoardIndex === -1 ? categoryToUpdate.boards.length -1 : targetBoardIndex;
                    categoryToUpdate.boards = arrayMove(categoryToUpdate.boards, sourceBoardIndex, finalTargetBoardIndex); 
                    newCategories[sourceCategoryIndex] = categoryToUpdate;
                    
                    const orderedBoardIds = categoryToUpdate.boards.map(board => board.id);
                    ...
                    const finalCategories = prevCategories.map((category, index) => {
                        ...
                        if (index === sourceCategoryIndex) {
                            const filteredBoards = category.boards.filter(board => board.id !== actualBoardToMove.id);
                            return { 
                                ...category, 
                        ...
                        if (index === targetCategoryIndex) {
                            const updatedMovedBoard = { ...actualBoardToMove, category_id: category.id }; // Update category_id
                            const targetIndex = targetBoardIndex === -1 ? category.boards.length : targetBoardIndex;
                    ...
                    // API Calls
                    const sourceBoardIds = finalCategories[sourceCategoryIndex].boards.map(board => board.id);
                    const targetBoardIds = finalCategories[targetCategoryIndex].boards.map(board => board.id);
                    
                    saveBoardOrder(sourceBoardIds); 
                    saveBoardOrder(targetBoardIds);
                    updateBoardCategory(actualBoardToMove.id, finalCategories[targetCategoryIndex].id); 
    ...
    const updateBoardCategory = async (boardId: string, targetCategoryId: string) => {
        if (!serverUrl) return;
        try {
            const authHeaders = await fetchHeaders();
            if (!authHeaders) throw new Error("No auth headers");
            ...
            const baseUrl = serverUrl.endsWith('/') ? serverUrl.slice(0, -1) : serverUrl;
            const response = await fetch(`${baseUrl}/forum/boards/${boardId}`, { // Use existing PUT endpoint
                method: 'PUT',
                headers: { ...authHeaders, 'Content-Type': 'application/json' },
    ...
    const findItemData = (id: string | null) => {
        ...
        if (id.startsWith('category-')) {
            ...
            return categories.flatMap(cat => cat.boards).find(board => board.id === boardId);
        }
        return null;
    };
        ...
        content = (
            ...
            >
                <div className="max-w-4xl mx-auto space-y-6">
                    ...
                    <form onSubmit={handleAddCategory} className="p-4 border rounded shadow-sm bg-gray-50 space-y-3">
                        <h3 className="text-lg font-medium">Add New Category</h3>
                        <div>
                            <label htmlFor="newCatName" className="block text-sm font-medium text-gray-700">Name</label>
                            <input 
                                id="newCatName"
                                type="text" 
                        ...
                        <div>
                            <label htmlFor="newCatDesc" className="block text-sm font-medium text-gray-700">Description</label>
                            <textarea 
                                id="newCatDesc"
                        ...
                        {addCategoryError && <p className="text-sm text-red-600">Error: {addCategoryError}</p>}
                        <button 
                            type="submit" 
                                    ...
                                    {(listeners, attributes) => (
                                        <div className="p-4 border rounded shadow-sm space-y-4 bg-white mb-4">
                                            {editingCategory && editingCategory.id === category.id ? (
                                                <form onSubmit={handleSaveCategory} className="space-y-3 mb-4">
                                                    <h3 className="text-lg font-medium">Editing Category: {editingCategory.name}</h3>
                                                    <div>
                                                        <label htmlFor={`editCatName-${category.id}`} className="block text-sm font-medium text-gray-700">Name</label>
                                                        <input 
                                                    ...
                                                    <div>
                                                        <label htmlFor={`editCatDesc-${category.id}`} className="block text-sm font-medium text-gray-700">Description</label>
                                                        <textarea 
                                                    ...
                                                    {saveCategoryError && <p className="text-sm text-red-600">Error: {saveCategoryError}</p>}
                                                ...
                                                <div className="flex justify-between items-center mb-2">
                                                    ...
                                                    <div className="flex space-x-2 flex-shrink-0">
                                                         <button
                                                            onClick={() => handleEditCategory({id: category.id, name: category.name, description: category.description, order: category.order})}
                                                            disabled={!!editingCategory || !!editingBoard || !!deletingCategoryId || !!deletingBoardId || isAddingBoard || isAddingCategory}
                                            ...
                                            {!(editingCategory && editingCategory.id === category.id) && (
                                                <> {/* Wrap both sections in a Fragment */} 
                                                    {/* --- Boards Section --- */}
                                                    <div>
                                                        <h4 className="text-md font-semibold mb-2 pl-5">Boards:</h4>
                                                        {category.boards.length === 0 ? (
                                                            <p className="text-sm text-gray-500 pl-5">No boards in this category.</p>
                                                        ) : (
                                                            <SortableContext
                                                                items={category.boards.map(board => `board-${board.id}`)}
                                                                strategy={verticalListSortingStrategy}
                                                            >
                                                                <ul className="space-y-1 pl-5 min-h-[2rem] py-1">
                                                                    {category.boards.length === 0 && (
                                                                        <li className="text-sm text-gray-400 italic pl-2">(No boards)</li>
                                                                    )}
                                                                    {category.boards.map((board) => (
                                                                        <SortableItem key={board.id} id={`board-${board.id}`}>
                                                                            {(boardListeners, boardAttributes) => (
                                                                               <li className="flex justify-between items-center group py-1.5 bg-gray-50 rounded px-2">
                                                                                   {editingBoard && editingBoard.id === board.id ? (
                                                                                       <form onSubmit={handleSaveBoard} className="space-y-3 mb-4">
                                                                                           <h3 className="text-lg font-medium">Editing Board: {board.name}</h3>
                                                                                           <div>
                                                                                               <label htmlFor={`editBoardName-${board.id}`} className="block text-sm font-medium text-gray-700">Name</label>
                                                                                               <input 
                                                                                           ...
                                                                                           <div>
                                                                                               <label htmlFor={`editBoardDesc-${board.id}`} className="block text-sm font-medium text-gray-700">Description</label>
                                                                                               <textarea 
                                                                                           ...
                                                                                           {saveBoardError && <p className="text-sm text-red-600">Error: {saveBoardError}</p>}
                                                                                           <div className="flex space-x-2">
                                                                                               <button 
                                                                                                   type="submit" 
                                                                                                   disabled={isSavingBoard || !editBoardName.trim()}
                                                                                                   className="inline-flex justify-center py-1 px-3 border border-transparent shadow-sm text-xs font-medium rounded-md text-white bg-green-600 hover:bg-green-700 focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-green-500 disabled:opacity-50 disabled:cursor-not-allowed"
                                                                                               >
                                                                                       ...
                                                                                       <>
                                                                                           {/* --- Pass listeners/attrs to DragHandle for Board --- */}
                                                                                            <DragHandle listeners={boardListeners} attributes={boardAttributes}/>
                                                                                           ...
                                                                                           <div className="flex space-x-2 flex-shrink-0">
                                                                                                <button
                                                                                                   ...
                                                                                                   disabled={!!editingBoard || !!deletingBoardId || isAddingBoard || isAddingCategory}
                                                                                                   className="text-blue-600 hover:text-blue-800 text-sm disabled:opacity-50 disabled:cursor-not-allowed"
                                                                                               >
                                                                                                   Edit
                                                                                               </button>
                                                    ...
                                                    <div className="pl-5 pt-2"> 
                                                        {addingBoardToCategoryId === category.id ? (
                                                            <form onSubmit={handleAddBoard} className="mt-4 p-3 border-t border-dashed space-y-2">
                                                                <h5 className="font-medium text-sm">Add New Board to "{category.name}"</h5>
                                                                <div>
                                                                    <label htmlFor={`newBoardName-${category.id}`} className="block text-sm font-medium text-gray-700">Name</label>
                                                                    <input 
                                                                ...
                                                                <div>
                                                                    <label htmlFor={`newBoardDesc-${category.id}`} className="block text-sm font-medium text-gray-700">Description</label>
                                                                    <textarea 
                                                                ...
                                                                {addBoardError && <p className="text-sm text-red-600">Error: {addBoardError}</p>}
                                                                <div className="flex space-x-2">
                                                                    <button 
                                                                        type="submit" 
                                                                        disabled={isAddingBoard || !newBoardName.trim() || !!editingCategory || !!editingBoard}
                                                            ...
                                                            <button
                                                                onClick={() => handleShowAddBoardForm(category.id)}
                                                                disabled={!!editingCategory || !!editingBoard || !!deletingCategoryId || !!deletingBoardId || isAddingBoard || isAddingCategory}
                ...
                {/* --- Drag Overlay --- */}
                {createPortal(
                     <DragOverlay dropAnimation={null}> {/* Optional: customize animation */} 
                             ...
                             activeId.startsWith('category-') ? (
                                 ...
                                 <div className="p-4 border rounded shadow-sm space-y-4 bg-white opacity-90">
                                     <div className="flex justify-between items-center mb-2">
                                         ...
                                         <div className="flex-grow mr-4">
                                             <h3 className="text-lg font-medium truncate">{(activeItemData as Category).name}</h3>
                                             <p className="text-sm text-gray-600 truncate">{(activeItemData as Category).description}</p>
                                         </div>
                                 ...
                                 <li className="flex justify-between items-center group py-1.5 bg-gray-100 rounded px-2 opacity-90 shadow-md">
                                     ...
                                     <div className="flex-grow mr-2 truncate">
                                         <span className="font-medium">{(activeItemData as Board).name}</span>
