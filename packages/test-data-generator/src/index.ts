...
async function fullSync(handle: Core.ProcessHandle.ProcessHandle) {
  while (await Core.Synchronization.backFillServers(handle, handle.system())) {}
}
...
async function scenarioEveryClaimType() {
  ...
  const claims = [
    ...
    await s1p1.claim(
      Core.Models.claimOccupation('FUTO', 'Software Engineer', 'Austin Texas'),
    ),
    await s1p1.claim(
      Core.Models.claimOccupation('FUTO', undefined, 'Austin Texas'),
    ),
    await s1p1.claim(Core.Models.claimOccupation('FUTO', undefined, undefined)),
    await s1p1.claim(
    ...
    await s1p1.claim(
      Core.Models.claimOccupation(undefined, 'Software Engineer', undefined),
    ),
  ];
