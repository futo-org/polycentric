...
const IframeEmbed = ({ src, ...rest }: { src: string } & React.IframeHTMLAttributes<HTMLIFrameElement>) => {
  return <iframe width="100%" height="auto" className="aspect-video" src={src} allowFullScreen {...rest} />
}
...
const VideoEmbed = ({ src, ...rest }: { src: string } & React.VideoHTMLAttributes<HTMLVideoElement>) => {
  return <video className="aspect-video" src={src} controls {...rest} />
}

const TopFeedVideoContainer = ({ children, platformName }: { children: React.ReactNode; platformName: string }) => {
