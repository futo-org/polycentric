...
describe('query event2', () => {
  ...
  test('hit network', async () => {
    ...
    expect(
      Models.Pointer.equal(messagePointer, Models.signedEventToPointer(result)),
    ).toStrictEqual(true);
  ...
  test('network queries combined', async () => {
    ...
    const getEvents = async (
      ...
      expect(Models.PublicKey.equal(system, s1p1.system())).toStrictEqual(true);
      ...
      expect(
        Models.Process.equal(
          Models.Process.fromProto(ranges.rangesForProcesses[0].process),
          s1p1.process(),
        ),
      ).toStrictEqual(true);
      ...
      const event1 = await s1p1
        .store()
        .indexEvents.getSignedEvent(e1.system, e1.process, e1.logicalClock);

      expectToBeDefined(event1);

      const event2 = await s1p1
        .store()
        .indexEvents.getSignedEvent(e2.system, e2.process, e2.logicalClock);
