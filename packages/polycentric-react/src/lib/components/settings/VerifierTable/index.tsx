...
import { useQueryVerifiers } from '../../../hooks/queryHooks';
...
interface ExistingVerifier {
  kind: 'existingServer';
  server: string;
}

interface NewVerifier {
  kind: 'newServer';
  close: () => void;
}

const VerifierListTableRow = ({
  params,
}: {
  params: ExistingVerifier | NewVerifier;
}) => {
      ...
      <button
        ...
        onClick={() => {
          if (params.kind === 'existingServer') {
            ...
            processHandle.removeVerifier(params.server);
          }
        }}
        disabled={mutationSubmitted}
      >
  ...
  const handleServerSubmit = () => {
    ...
    // Add the server
    processHandle.addVerifier(serverUrl);
  };
...
export const VerifierListTable = () => {
  const { processHandle } = useProcessHandleManager();
