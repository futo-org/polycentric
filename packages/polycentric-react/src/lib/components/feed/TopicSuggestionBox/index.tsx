...
const getRelevantTopics = (query: string, topics: InfiniteMap, out: InfiniteMap = {}): void => {
  // Find all branches on the tree and below that matches the query
  // Ex. /tpot/dating => tpot: {  dating: { foo: {}}, },

  const [root, rest] = query.split('/')
  const matchingBranches = Object.keys(topics).filter((t) => t.startsWith(root))
    ...
    {Object.keys(topics).map((topic) => {
      ...
      return (
        <div key={newPath}>
          ...
          <div className="ml-12">
            <TopicSuggestionBoxPill topics={topics[topic]} path={newPath} setSelected={setSelected} />
          </div>
        </div>
      )
    })}
...
export const TopicSuggestionBox = ({
  ...
  return (
    <div className="bg-white p-5 rounded-t-sm rounded-b-xl border-t border-b-2 border-x-2 border-gray-300 w-full">
