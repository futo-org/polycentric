...
async fn test_websocket_manager_registration() {
    ...
    ws_manager.register_connection(
        connection_id,
        identity.polycentric_identity.clone(),
        tx,
    ).await;

    // Test user is online
    let is_online = ws_manager.is_user_online(&identity.polycentric_identity).await;
    assert!(is_online);

    // Test connection count
    let count = ws_manager.get_user_connection_count(&identity.polycentric_identity).await;
    assert_eq!(count, 1);
...
async fn test_websocket_message_sending() {
    ...
    ws_manager.send_to_user(&identity.polycentric_identity, test_message.clone()).await;
    ...
    if let Some(Message::Text(text)) = received {
        ...
        match parsed {
            WSMessage::ConnectionAck { connection_id: received_id } => {
...
async fn test_websocket_send_to_specific_connection() {
    ...
    ws_manager.send_to_connection(connection_id, test_message).await;
...
async fn test_websocket_broadcast() {
    ...
    ws_manager.register_connection(
        connection_id1,
        identity1.polycentric_identity.clone(),
        tx1,
    ).await;

    ws_manager.register_connection(
        connection_id2,
        identity2.polycentric_identity.clone(),
        tx2,
    ).await;
...
async fn test_websocket_manager_stats() {
    ...
    ws_manager.register_connection(connection_id1, identity1.polycentric_identity.clone(), tx1).await;
    ws_manager.register_connection(connection_id2, identity1.polycentric_identity.clone(), tx2).await;
    ws_manager.register_connection(connection_id3, identity2.polycentric_identity.clone(), tx3).await;
    ...
    if let Some(Message::Text(text)) = received {
        ...
        match parsed {
            WSMessage::DMMessage { message } => {
                ...
                assert_eq!(message.recipient.key_bytes, recipient.polycentric_identity.key_bytes);
            }
            _ => panic!("Wrong message type received"),
        }
    } else {
...
async fn test_websocket_typing_indicator() {
    ...
    ws_manager.register_connection(
        connection_id,
        user.polycentric_identity.clone(),
        tx,
    ).await;
    ...
    ws_manager.send_to_user(&user.polycentric_identity, typing_message).await;
...
async fn test_websocket_read_receipt() {
    ...
    ws_manager.send_to_user(&user.polycentric_identity, read_receipt).await;
    ...
    if let Some(Message::Text(text)) = received {
        let parsed: WSMessage = serde_json::from_str(&text).unwrap();
        match parsed {
            WSMessage::ReadReceipt { message_id, reader, read_timestamp: _ } => {
...
async fn test_websocket_error_message() {
    ...
    ws_manager.send_to_connection(connection_id, error_message).await;
