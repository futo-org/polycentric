...
impl PointerBatch {
    ...
    pub (crate) fn append(
        ...
        self.p_subject_system_key_type.push(
            i64::try_from(crate::model::public_key::get_key_type(
                pointer.system(),
            ))?,
        );

        self.p_subject_system_key.push(
            crate::model::public_key::get_key_bytes(pointer.system())
        );

        self.p_subject_process.push(
            pointer.process().bytes().to_vec(),
        );

        self.p_subject_logical_clock.push(
            i64::try_from(*pointer.logical_clock())?,
        );

        self.p_link_content_type.push(
            i64::try_from(link_content_type)?,
        );
