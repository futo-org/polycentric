...
use dm_server::{
    ...
    models::{WSMessage, DMMessageResponse},
};
...
async fn test_full_message_flow() {
    ...
    let history = setup.db.get_dm_history(
        &sender.polycentric_identity,
        &recipient.polycentric_identity,
        None,
        10,
    ...
    // Test decryption
    let decrypted = MessageHelper::decrypt_test_message(
        &recipient,
        &ephemeral_key,
        &encrypted_content,
        &nonce,
...
async fn test_websocket_message_delivery() {
    ...
    ws_manager.register_connection(
        connection_id,
        recipient.polycentric_identity.clone(),
        tx,
    ).await;
    ...
    ws_manager.send_to_user(&recipient.polycentric_identity, ws_message).await;
    ...
    if let Some(tokio_tungstenite::tungstenite::Message::Text(text)) = received {
        ...
        match parsed {
            WSMessage::DMMessage { message } => {
                ...
                assert_eq!(message.sender.key_bytes, sender.polycentric_identity.key_bytes);
            }
...
async fn test_user_registration_and_messaging_flow() {
    ...
    let alice_history = setup.db.get_dm_history(
        &alice.polycentric_identity,
        &bob.polycentric_identity,
        None,
        10,
    ...
    let bob_history = setup.db.get_dm_history(
        &bob.polycentric_identity,
        &alice.polycentric_identity,
        None,
        10,
    ...
    let decrypted1 = MessageHelper::decrypt_test_message(
        &bob,
        &eph_key1,
        &enc_content1,
        &nonce1,
    ...
    let decrypted2 = MessageHelper::decrypt_test_message(
        &alice,
        &eph_key2,
        &enc_content2,
        &nonce2,
    ...
    let conversations = setup.db.get_conversation_list(&alice.polycentric_identity, 10).await.unwrap();
    assert_eq!(conversations.len(), 1);
    assert_eq!(conversations[0].0.key_bytes, bob.polycentric_identity.key_bytes);
}
...
async fn test_message_delivery_tracking() {
    ...
    setup.db.register_x25519_key(&sender.polycentric_identity, &sender.x25519_public_key, &sender_sig).await.unwrap();
    setup.db.register_x25519_key(&recipient.polycentric_identity, &recipient.x25519_public_key, &recipient_sig).await.unwrap();
...
async fn test_undelivered_messages_retrieval() {
    ...
    for i in 0..3 {
        ...
        if i == 0 {
            setup.db.mark_message_delivered(&message_id, base_time + chrono::Duration::seconds(10)).await.unwrap();
        }
    }

    // Get undelivered messages
    let undelivered = setup.db.get_undelivered_messages(
        &recipient.polycentric_identity,
...
async fn test_multiple_device_connections() {
    ...
    ws_manager.register_connection(device1_id, user.polycentric_identity.clone(), tx1).await;
    ws_manager.register_connection(device2_id, user.polycentric_identity.clone(), tx2).await;
    ws_manager.register_connection(device3_id, user.polycentric_identity.clone(), tx3).await;

    // Register in database
    setup.db.register_connection(device1_id, &user.polycentric_identity, Some("Mobile App")).await.unwrap();
    setup.db.register_connection(device2_id, &user.polycentric_identity, Some("Desktop App")).await.unwrap();
    setup.db.register_connection(device3_id, &user.polycentric_identity, Some("Web Browser")).await.unwrap();

    // Verify connection count
    let count = ws_manager.get_user_connection_count(&user.polycentric_identity).await;
    assert_eq!(count, 3);

    let db_connections = setup.db.get_user_connections(&user.polycentric_identity).await.unwrap();
    assert_eq!(db_connections.len(), 3);

    // Send message to all devices
    let test_message = WSMessage::Ping;
    ws_manager.send_to_user(&user.polycentric_identity, test_message).await;
...
async fn test_connection_cleanup() {
    ...
    // Register connection
    setup.db.register_connection(connection_id, &user.polycentric_identity, None).await.unwrap();
    ...
    let connections = setup.db.get_user_connections(&user.polycentric_identity).await.unwrap();
...
async fn test_message_pagination() {
    ...
    setup.db.register_x25519_key(&alice.polycentric_identity, &alice.x25519_public_key, &alice_sig).await.unwrap();
    setup.db.register_x25519_key(&bob.polycentric_identity, &bob.x25519_public_key, &bob_sig).await.unwrap();
    ...
    let page1 = setup.db.get_dm_history(
        &alice.polycentric_identity,
        &bob.polycentric_identity,
        None,
        5,
    ...
    let page2 = setup.db.get_dm_history(
        &alice.polycentric_identity,
        &bob.polycentric_identity,
        Some(&cursor),
        5,
    ...
    let page3 = setup.db.get_dm_history(
        &alice.polycentric_identity,
        &bob.polycentric_identity,
        Some(&cursor),
        5,
