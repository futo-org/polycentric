    ...
    const interval = setInterval(() => {
      if (Notification.permission === 'granted') {
        ...
        navigator.storage.persist().then((successfullyPersisted) => {
          setState(successfullyPersisted ? 'persisted' : 'persist_call_failed');
        });
      }
    }, 1000);
  ...
  return (
    <OnboardingPanel imgSrc={starterURL}>
      <div className="flex flex-col p-10 gap-y-10 justify-center h-full">
        ...
        <button
          disabled={state === 'persist_call_failed' || state === 'persisted'}
          ...
          onClick={async () => {
            ...
            if (permission === 'denied') {
              ...
              navigator.storage.persist().then((successfullyPersisted) => {
                setState(
                  successfullyPersisted ? 'persisted' : 'persist_call_failed',
                );
              });
            }
          }}
  ...
  return (
    <OnboardingPanel imgSrc={starterURL}>
      <div className="flex flex-col p-10 gap-y-10 justify-center h-full">
        ...
        <button
          disabled={state === 'persisted' || state === 'persist_call_failed'}
          className="bg-blue-500 disabled:bg-blue-200 text-white border rounded-full md:rounded-md py-2 px-4 font-bold text-lg"
          onClick={() => {
            navigator.storage.persist().then((persisted) => {
              setState(persisted ? 'persisted' : 'persist_call_failed');
            });
          }}
        >
          {state === 'persist_call_failed'
