        ...
        async () => {
            ...
            function referenceToString(reference: Protocol.Reference): string {
                return Base64.encode(
                    Protocol.Reference.encode(reference).finish(),
                );
            }
            ...
            function checkResult(
                ...
                const item = referenceToItem.get(
                    referenceToString(rootPosts[i]),
                );
                expect(item).toBeDefined();
                expect(item!.counts[0]).toStrictEqual(new Long(likes, 0, true));
                expect(item!.counts[1]).toStrictEqual(
                    new Long(dislikes, 0, true),
                );
                expect(item!.counts[2]).toStrictEqual(
                    new Long(replies, 0, true),
                );
            }
