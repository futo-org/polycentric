...
export const usePostStatsWithLocalActions = (pointer: Models.Pointer.Pointer) => {
  ...
  const [opinion, setOpinion] = useState<'liked' | 'neutral' | 'disliked'>('neutral')
  ...
  const locallyModifiedDislikes = useMemo(() => {
    ...
    } else if (opinion === 'disliked' && likedOnMount === false && stats.dislikes) {
      dislikes = stats.dislikes + 1
