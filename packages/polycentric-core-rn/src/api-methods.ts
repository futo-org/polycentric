...
// Use native fetch in React Native instead of cross-fetch
...
// Enhanced fetch with timeout for React Native
...
// Enhanced options for API requests
function enhanceRequestOptions(options: RequestInit, isPost: boolean = false): RequestInit {
  const headers = new Headers(options.headers || {});
  
  // Add common headers
  headers.set('x-polycentric-user-agent', userAgent);
  ...
  // Add accept header for GET requests
  if (!isPost && !headers.has('Accept')) {
...
export async function postEvents(
  ...
  const response = await fetchWithRetry(server + '/events', enhanceRequestOptions({
    method: 'POST',
...
export async function getRanges(
  ...
  const response = await fetchWithRetry(server + path, enhanceRequestOptions({
    method: 'GET',
  }));
