...
class SoundCloudTextVerifier extends TextVerifier {
    ...
    private async getTextFromInternalId(id: string): Promise<Result<string>> {
        const client = createCookieEnabledAxios();
        const homeResponse = await this.callUrl(client, `https://soundcloud.com/discover`, true);
        ...
        const clientIdMatch = homeResponse.data.match(/"clientId":"([a-zA-Z0-9-_]+)"/);
        if (!clientIdMatch) {
        ...
        if (profileResponse.status !== 200) {
            return Result.err({
                message: 'Unable to find your SoundCloud account',
                extendedMessage: `Failed to get API profile page (${profileResponse.status}): '${
                    profileResponse.statusText
