...
export function concatBuffers(buffers: readonly Uint8Array[]): Uint8Array {
  const result = new Uint8Array(buffers.reduce((acc, x) => acc + x.length, 0));
    ...
    const queryServersSubscription =
      Queries.QueryServers.queryServersObservable(
        ...
        taskPerItemInSet(
          ...
          identity,
          (server) => {
            const taskSubscription = task(server).subscribe((value) => {
              subscriber.next(value);
            });

            return taskSubscription.unsubscribe.bind(taskSubscription);
          },
          (unsubscribe) => {
            unsubscribe();
          },
