 ...
 */
export function useAuthHeaders(serverUrl: string | undefined): UseAuthHeadersResult {
    ...
    const fetchHeaders = useCallback(async (): Promise<AuthHeaders | null> => {
        ...
        try {
            ...
            const challengeResRaw = await fetch(challengeUrl, { credentials: 'include' });
            if (!challengeResRaw.ok) throw new Error(`Auth header prep failed (challenge @ ${serverUrl}): ${challengeResRaw.status} ${challengeResRaw.statusText}`);
            ...
            try {
                ...
                if (!challengeData || typeof challengeData.challenge_id !== 'string' || typeof challengeData.nonce_base64 !== 'string') {
                     throw new Error('Incomplete challenge data received');
                }
            } catch (jsonError) {
                console.error(`Error parsing JSON for challenge from ${serverUrl}:`, jsonError);
                console.error(`Raw challenge response text from ${serverUrl}:`, challengeResText);
                throw new Error(`Invalid JSON challenge response received from ${serverUrl}`);
            }
            ...
            if (!privateKey) throw new Error('Private key unavailable for signing challenge.');
            ...
            console.error(`Error preparing auth headers for ${serverUrl}:`, fetchError);
            setError(fetchError.message || `Failed to prepare auth headers for ${serverUrl}.`);
            setHeaders(null);
            return null;
        } finally {
            setLoading(false);
