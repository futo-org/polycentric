  ...
  public async query(
    ...
    await Promise.all(
      keys.map(async (key) => {
        ...
        const following = await this.store.indexCRDTElementSet.queryIfAdded(
          storeSystem,
        ...
        if (following) {
          const signedEvent = await this.store.indexEvents.getSignedEventByKey(
    ...
    return {
      items: result,
      cursor:
        !queryEnded && keys.length > 1 ? keys[keys.length - 1] : undefined,
    };
  }
}
