use entity::{content, content_delete, content_post, event, quote};
...
impl MigrationTrait for Migration {
    async fn up(&self, manager: &SchemaManager) -> Result<(), DbErr> {
        if manager.has_table(quote::Entity.unquoted()).await? {
            return Ok(());
        }
        ...
        create_table
            .table(quote::Entity.unquoted())
            .col({
                let mut def = ColumnDef::new(quote::Column::EventId);
                def.primary_key().big_integer().not_null();
                def
            })
            .col({
                let mut def = ColumnDef::new(quote::Column::Identity);
                def.text().not_null();
                def
            })
            .col({
                let mut def = ColumnDef::new(quote::Column::Post);
        ...
        fill_table
            ...
            .select_from({
                ...
                q
                    ...
                    .expr(SelectExpr {
                        expr: Expr::col((
                              "quote_event", event::Column::Id.unquoted(),
                        )),
                    ...
                    .inner_join(
                        TableRef::Table(event::Entity.into(), Some("quote_event".into())),
                        Condition::all()
                            .and(Expr::col(content_post::Column::QuoteCollection.as_column_ref())
                                .eq(Expr::col(("quote_event", event::Column::Collection))))
                            .and(Expr::col(content_post::Column::QuoteIdentity.as_column_ref())
                                .eq(Expr::col(("quote_event", event::Column::Identity))))
                            .and(Expr::col(content_post::Column::QuotePublicKeyType.as_column_ref())
                                .eq(Expr::col(("quote_event", event::Column::PublicKeyType))))
                            .and(Expr::col(content_post::Column::QuotePublicKey.as_column_ref())
                                .eq(Expr::col(("quote_event", event::Column::PublicKey))))
                            .and(Expr::col(content_post::Column::QuoteSequence.as_column_ref())
                                .eq(Expr::col(("quote_event", event::Column::Sequence))))
                    )
                    .left_join(
            ...
            .on_conflict({
                let mut c = OnConflict::column(quote::Column::EventId);
                c.do_nothing();
                c
            });
    ...
    async fn down(&self, manager: &SchemaManager) -> Result<(), DbErr> {
        ...
        drop_table.table(quote::Entity.unquoted()).restrict();
