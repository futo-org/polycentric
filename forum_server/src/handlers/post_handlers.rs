...
use crate::{
    ...
    repositories::{
        post_repository::{self, UpdatePostData},
        thread_repository,
    },
...
pub async fn create_post_handler(
        ...
        match multipart.next_field().await {
            Ok(Some(field)) => {
                ...
                match field_name.as_str() {
                    ...
                    "quote_of" => match field.bytes().await {
                        Ok(data) => match String::from_utf8(data.to_vec()) {
                            Ok(value_str) => {
                                if !value_str.is_empty() {
                                    match Uuid::parse_str(&value_str) {
                                        ...
                                        Err(_) => {
                                            return (
                                                StatusCode::BAD_REQUEST,
                                                "Invalid UUID format for quote_of",
                                            )
                                                .into_response()
                                        }
                                    }
                            ...
                            Err(_) => {
                                return (StatusCode::BAD_REQUEST, "Invalid UTF-8 in quote_of field")
                                    .into_response()
                            }
                        },
                        Err(e) => {
                            return (
                                StatusCode::BAD_REQUEST,
                                format!("Failed to read quote_of field: {}", e),
                            )
                                .into_response()
                        }
                    },
                    ...
                    "polycentric_system_id" => match field.text().await {
                        ...
                        Err(e) => {
                            return (
                                StatusCode::BAD_REQUEST,
                                format!("Failed to read polycentric_system_id: {}", e),
                            )
                                .into_response()
                        }
                    },
                    "polycentric_process_id" => match field.text().await {
                        Ok(text) => collected_poly_process_id_b64 = Some(text),
                        Err(e) => {
                            return (
                                StatusCode::BAD_REQUEST,
                                format!("Failed to read polycentric_process_id: {}", e),
                            )
                                .into_response()
                        }
                    },
                    "polycentric_log_seq" => match field.text().await {
                        Ok(text) => collected_poly_log_seq_str = Some(text),
                        Err(e) => {
                            return (
                                StatusCode::BAD_REQUEST,
                                format!("Failed to read polycentric_log_seq: {}", e),
                            )
                                .into_response()
                        }
                    },
    ...
    if polycentric_system_id.is_some() != polycentric_process_id.is_some()
        || polycentric_system_id.is_some() != polycentric_log_seq.is_some()
    {
        return (
            StatusCode::BAD_REQUEST,
    ...
    match thread_repository::get_thread_by_id(&state.db_pool, thread_id).await {
        ...
        Err(e) => {
            ...
            return (
                ...
                "Error checking thread existence",
            )
                .into_response();
        }
    }
...
pub async fn update_post_handler(
    ...
    match post_repository::get_post_by_id(&state.db_pool, post_id).await {
        ...
        Err(e) => {
            ...
            (
                StatusCode::INTERNAL_SERVER_ERROR,
                "Error fetching post for update",
            )
                .into_response()
        }
    }
    ...
    if can_delete {
        let thread_id = match post_repository::get_post_thread_id(&state.db_pool, post_id).await {
            ...
            Err(e) => {
                ...
                return (
                    StatusCode::INTERNAL_SERVER_ERROR,
                    "Database error getting thread ID",
                )
                    .into_response();
            }
        };
        ...
        match post_repository::delete_post(&state.db_pool, post_id).await {
            Ok(1) => {
                ...
                match post_repository::count_posts_in_thread(&state.db_pool, thread_id).await {
                    Ok(0) => {
                        match thread_repository::delete_thread_with_posts(&state.db_pool, thread_id)
                            .await
                        {
                            Ok(_) => {
...
pub async fn link_polycentric_post_handler(
    ...
    match post_repository::get_post_author(&state.db_pool, post_id).await {
        Ok(Some(author_id)) => {
            if author_id != user_pubkey {
                ...
                return (
                    ...
                    "Permission denied. You can only link your own posts.",
                )
                    .into_response();
            }
        }
        ...
        Err(e) => {
            ...
            return (
                StatusCode::INTERNAL_SERVER_ERROR,
                "Database error checking author.",
            )
                .into_response();
        }
    }
    ...
    let system_id = match BASE64_STANDARD.decode(&payload.polycentric_system_id_b64) {
        ...
        Err(_) => {
            ...
            return (
                StatusCode::BAD_REQUEST,
                "Invalid base64 format for polycentric_system_id",
            )
                .into_response();
        }
    };
    let process_id = match BASE64_STANDARD.decode(&payload.polycentric_process_id_b64) {
        ...
        Err(_) => {
            ...
            return (
                StatusCode::BAD_REQUEST,
                "Invalid base64 format for polycentric_process_id",
            )
                .into_response();
        }
    };
