import { CancelContext, Models, Protocol, Queries, Util } from '@polycentric/polycentric-core'
import Long from 'long'
import { useCallback, useEffect, useMemo, useState } from 'react'
import { ParsedEvent, useIndex, useQueryCursor, useQueryManager, useQueryReferenceEventFeed } from './queryHooks'
...
export const useCommentFeed = (
  ...
  const [backwardsChain, setBackwardsChain] = useState<ParsedEvent<Protocol.Post>[]>([])
  ...
  const fetchPost = useCallback(
    (
      ...
      queryManager.queryEvent.query(system, process, logicalClock, (signedEvent) => {
  ...
  const prependCount = useMemo(() => (backwardsChain.length > 0 ? backwardsChain.length - 1 : 0), [backwardsChain])

  const all = useMemo(() => [...backwardsChain, ...comments], [backwardsChain, comments])
