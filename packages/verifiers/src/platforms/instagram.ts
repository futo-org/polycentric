...
class InstagramOAuthVerifier extends OAuthVerifier<InstagramTokenRequest> {
    ...
    public async getOAuthURL(): Promise<Result<string>> {
        if (process.env.INSTAGRAM_CLIENT_ID === undefined || process.env.OAUTH_CALLBACK_DOMAIN === undefined) {
    ...
    public async getToken(data: InstagramTokenRequest): Promise<Result<TokenResponse>> {
        if (
        ...
        const tokenResponse = await client.post('https://api.instagram.com/oauth/access_token', form);

        if (tokenResponse.status !== StatusCodes.OK) {
            return httpResponseToError(tokenResponse.status, tokenResponse.data, 'Instagram API /oauth/access_token');
        }
        ...
        return httpResponseToError(response.status, response.data, 'Instagram API /me');
    }
