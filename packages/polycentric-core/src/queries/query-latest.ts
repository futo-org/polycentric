...
function makeAttemptedBatch(
  ...
  for (const signedEvent of signedEvents) {
    ...
    let batchByContentType: BatchForContentTypeState | undefined = undefined;
...
export class QueryLatest implements HasUpdate {
  ...
  public query(
    ...
    return () => {
      ...
      this.cleanup(system, contentType, stateForSystem, stateForContentType);
    };
  }
  ...
  private loadFromDisk(
    ...
    const loadFromDisk = (process: Models.Process.Process) =>
      RXJS.from(
        this.index.getLatest(system, process, stateForContentType.contentType),
      );
    ...
    const getLatest = (headSignedEvent: Models.SignedEvent.SignedEvent) => {
      ...
      if (headEvent.contentType.equals(stateForContentType.contentType)) {
        ...
        Models.Event.lookupIndex(headEvent, stateForContentType.contentType)
      ) {
    ...
    return QueryHead.queryHeadObservable(this.queryHead, system).pipe(
      ...
      RXJS.switchMap((signedEvents) => RXJS.of(makeAttempt(signedEvents))),
    );
  }
  ...
  private loadFromNetwork(
    ...
    const loadFromServer = async (server: string) => {
      ...
      return makeAttemptedBatch(server, needToUpdateStates, response.events);
    };
  ...
  public updateBatch(
    ...
    for (const signedEvent of signedEvents) {
      ...
      const contentTypeString = Models.ContentType.toString(event.contentType);
      ...
      const latestForProcess = stateForContentType.values.get(processString);

      if (
        !latestForProcess ||
        event.logicalClock.greaterThan(
          Models.Event.fromBuffer(latestForProcess.event).logicalClock,
        )
      ) {
