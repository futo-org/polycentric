    ...
    await new Promise<void>((resolve) => {
      ...
      const loadCallback: QueryCursor.LoadCallback = async (
        ...
        if (stage === 0) {
          ...
          return makeResult(signedEvents.slice(0, 10), new Uint8Array([1]));
        } else if (stage === 1) {
          expect(cursor).toStrictEqual(new Uint8Array([1]));

          return makeResult(signedEvents.slice(10, 20), new Uint8Array([2]));
        } else if (stage === 2) {
          expect(cursor).toStrictEqual(new Uint8Array([2]));

          return makeResult(signedEvents.slice(20), new Uint8Array([2]));
        } else {
          expect(cursor).toStrictEqual(new Uint8Array([2]));
