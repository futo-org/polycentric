...
class SpotifyOAuthVerifier extends OAuthVerifier<SpotifyTokenRequest> {
    ...
    public async getOAuthURL(): Promise<Result<string>> {
        if (process.env.SPOTIFY_CLIENT_ID === undefined || process.env.OAUTH_CALLBACK_DOMAIN === undefined) {
    ...
    public async getToken(data: SpotifyTokenRequest): Promise<Result<TokenResponse>> {
        if (
        ...
        const fdata = { code: data.code, redirect_uri: redirectUri, grant_type: 'authorization_code' };
        const result = await client({
            url: 'https://accounts.spotify.com/api/token',
            method: 'POST',
            data: qs.stringify(fdata),
            headers: {
                Authorization: `Basic ${Buffer.from(process.env.SPOTIFY_CLIENT_ID + ':' + process.env.SPOTIFY_CLIENT_SECRET).toString(
                    'base64'
                )}`,
        ...
        if (result.status !== StatusCodes.OK) {
            return httpResponseToError(result.status, result.data, 'Spotify API /api/token');
        }
        ...
        if (usernameResponse.status !== StatusCodes.OK) {
            return httpResponseToError(usernameResponse.status, usernameResponse.data, 'Spotify API /v1/me');
        }

        return Result.ok({ token: result.data.access_token, username: usernameResponse.data.display_name });
    }
    ...
    public async isTokenValid(challengeResponse: string, claimFields: ClaimField[]): Promise<Result<void>> {
        ...
        if (response.status !== StatusCodes.OK) {
            return httpResponseToError(response.status, response.data, 'Spotify API /v1/me');
        }
