...
async function setAvatarImage(
  ...
  for (const resolution of resolutions) {
    const imageBuffer = await Sharp(path).resize(resolution).jpeg().toBuffer();
...
describe('integration', () => {
  test('resolveAndQuery', async () => {
    ...
    await s1p1.claim(Models.claimBitcoin('1EaEv8DBeFfg6fE6BimEmvEFbYLkhpcvhj'));
  ...
  test('like', async () => {
    ...
    expect(
      Models.Opinion.equal(
        await bernstein.store().indexOpinion.get(bernstein.system(), subject),
  ...
  test('search', async () => {
    ...
    const newUsername = 'South Eastern Pennsylvania Transportation Authority';
  ...
  test('purge', async () => {
    ...
    const solvedChallenge = await ProcessHandle.solveChallenge(s1p1, challenge);
  ...
  test('claim and search identity handles', async () => {
    ...
    const contoso = await createHandleWithNameAndIdentityHandle(handleContoso1);
    const contoso2 =
    ...
    try {
      await APIMethods.postClaimHandle(TEST_SERVER, {
        handle:
          'This has spaces, dollar $ign$, and an &mpersand. not allowed!!',
