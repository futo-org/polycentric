...
export namespace ContentType {
  ...
  export const ContentTypeBlockTopic = makeContentType(19);
}
...
export namespace Digest {
  ...
  export function equal(a: Digest, b: Digest): boolean {
    return (
      a.digestType.equals(b.digestType) && Util.buffersEqual(b.digest, b.digest)
    );
  }
}
...
export namespace ResultEventsAndRelatedEventsAndCursor {
    ...
    return {
      ...
      resultEvents: {
        events: response.items
          .map((i) => i.event)
          .filter((e): e is SignedEvent.SignedEvent => e !== undefined),
      },
      relatedEvents: {
  ...
  export function equal(x: Type, y: Type): boolean {
    ...
    for (let i = 0; i < x.resultEvents.events.length; i++) {
      if (
        !SignedEvent.equal(x.resultEvents.events[i], y.resultEvents.events[i])
      ) {
        return false;
      }
    }
    ...
    for (let i = 0; i < x.relatedEvents.events.length; i++) {
      if (
        !SignedEvent.equal(x.relatedEvents.events[i], y.relatedEvents.events[i])
      ) {
        return false;
      }
    }
