...
class OdyseeTextVerifier extends TextVerifier {
  ...
  protected async verify(
    ...
    try {
      const expectedPublicKey = Buffer.from(pointer.system.key).toString(
        'base64',
      );
      ...
      for (const claimField of claimFields) {
        ...
        if (!textResult.success) {
          console.log(
            `Failed to get text for claim field: ${claimField.key}, ${claimField.value}`,
          );
          continue;
        }
        ...
        if (!text) {
          console.log(
            `Text is empty or undefined for claim field: ${claimField.key}, ${claimField.value}`,
          );
          continue;
        }
      ...
      if (isVerified) {
        ...
        return Result.err({
          ...
          extendedMessage:
            'The verification token was not found in your Odysee description.',
        });
      }
    } catch (error) {
      const errorMessage =
