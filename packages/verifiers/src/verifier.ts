...
export abstract class Verifier {
  ...
  public async requestVouch(
    ...
    if (contentType === 'application/json') {
      ...
      protocolPointer = Core.Protocol.Pointer.decode(
    ...
    if (ev.contentType.notEquals(Core.Models.ContentType.ContentTypeClaim)) {
      return Result.err({
        message: 'requestVouch: This event is not a claim event.',
      });
    }
    ...
    const shouldVouchForResult: Result = await this.shouldVouchFor(
      pointer,
      claim,
      req,
    );
...
export abstract class OAuthVerifier<T> extends Verifier {
  ...
  public abstract getOAuthURL(
    redirectUri: string,
  ): Promise<Result<string | XOAuthURLResult>>;
  ...
  protected async shouldVouchFor(
    ...
    const fields: ClaimField[] = claim.claimFields.map(
      (v) => <ClaimField>{ key: v.key.toInt(), value: v.value },
    );
...
export abstract class TextVerifier extends Verifier {
  ...
  protected async shouldVouchFor(
    ...
    const expectedPublicKey = Buffer.from(claimPointer.system.key).toString(
      'base64',
    );
    console.info(`Expected public key: '${expectedPublicKey}'.`);

    for (const claimField of claim.claimFields) {
      const descriptionResult = await this.getText({
        key: claimField.key.toInt(),
        value: claimField.value,
      });
