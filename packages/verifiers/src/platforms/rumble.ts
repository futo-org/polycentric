...
class RumbleTextVerifier extends TextVerifier {
    ...
    private async getTextFromChannel(id: string): Promise<Result<string>> {
        ...
        const profileResult = await client({ url: `https://rumble.com/c/${id}/about` });
    ...
    private async getTextFromUser(id: string): Promise<Result<string>> {
        const client = createCookieEnabledAxios();
        const profileResult = await client({ url: `https://rumble.com/user/${id}` });
        if (profileResult.status !== 200) {
            return Result.err({
                message: 'Unable to find your account',
                extendedMessage: `Failed to get Profile page (${profileResult.status}): '${
    ...
    public async getClaimFieldsByUrl(url: string): Promise<Result<ClaimField[]>> {
        const userMatch = /https:\/\/(?:www\.)?rumble\.com\/user\/([^/]+)\/?/.exec(url);
        if (userMatch) {
            return Result.ok([
                {
                    key: 0,
        ...
        const channelMatch = /https:\/\/(?:www\.)?rumble\.com\/c\/([^/]+)\/?/.exec(url);
        if (channelMatch) {
            return Result.ok([
                {
                    key: 1,
