    ...
    describe(currentTest.platform.name, () => {
        for (const verifier of currentTest.platform.verifiers) {
            if (verifier instanceof TextVerifier) {
                test(`(${currentTest.ci ? 'CI' : 'LOCAL'}) ${verifier.verifierType} health check`, async () => {
                    await verifier.init();
...
describe('requestVouch', () => {
    test(`text success`, async () => {
        ...
        const expectedPublicKey = Buffer.from(handle.system().key).toString('base64');
        const claimPointer = await handle.claim(Core.Models.claimHackerNews('test'));
        await Core.Synchronization.backFillServers(handle, handle.system());

        const verifier: TextVerifier = new DummyVerifier(Core.Models.ClaimType.ClaimTypeHackerNews, expectedPublicKey);
        await verifier.init();
    ...
    test(`text fail`, async () => {
        ...
        const verifier: TextVerifier = new DummyVerifier(Core.Models.ClaimType.ClaimTypeBitcoin, expectedPublicKey);
        await verifier.init();
        const vouchResult = await verifier.requestVouch(handle, {
            body: Core.Protocol.Pointer.encode(claimPointer).finish(),
            headers: {
