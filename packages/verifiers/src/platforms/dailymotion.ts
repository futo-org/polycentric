...
class DailymotionTextVerifier extends TextVerifier {
  ...
  protected async getText(claimField: ClaimField): Promise<Result<string>> {
    if (
      process.env.DAILYMOTION_CLIENT_ID === undefined ||
      process.env.DAILYMOTION_CLIENT_SECRET === undefined
    ) {
    ...
    if (anonymousTokenResult.status !== StatusCodes.OK) {
      return httpResponseToError(
        anonymousTokenResult.status,
        anonymousTokenResult.data,
        'Dailymotion API /oauth/access_token',
      );
    }
