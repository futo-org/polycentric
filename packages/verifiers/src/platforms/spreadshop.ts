...
class SpreadshopTextVerifier extends TextVerifier {
    ...
    protected async getText(claimField: ClaimField): Promise<Result<string>> {
        ...
        const profileResponse = await client.get(`https://${claimField.value}.myspreadshop.com/about`);
        ...
        if (!descriptionBox) {
            return Result.err({
                ...
                extendedMessage: 'Unable to find item with .sprd-about-info__text css class on webpage',
            });
        }
